...
REGION := $(shell cat $(STACK_CONFIG) | grep Region | cut -d = -f 2)
...
PARAMETERS := $(shell cat $(STACK_CONFIG) | grep -e LambdaRoleArn | tr '\n' ' ')
...
deploy: sam.yml
	aws cloudformation deploy \
		...
		--capabilities CAPABILITY_IAM $(PARAMETERS_OVERRIDES)
